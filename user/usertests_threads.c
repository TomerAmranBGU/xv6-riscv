...
void create_thread_exit_simple_other_thread_func() {
  print("blark is %d fun address is %p\n", kthread_id(),create_thread_exit_simple_other_thread_func);
  print("hello from other thread");
  kthread_exit(6);
}
void create_thread_exit_simple(char *s) {
  void *stack = malloc(4000);
  print("dad is %d function address is %p\n", kthread_id(), create_thread_exit_simple);
