...
int exec(char *path, char **argv)
{
  ...
  for (int i = 0; i < NTHREADS; i++)
  {
    ...
    p->threads[i].killed = 1;
    if(p->threads[i].state == T_SLEEPING){
      p->threads[i].state = T_RUNNABLE;
    }
  ...
  for(int i = 0; i < NTHREADS; i++){
    ...
    //todo add kthread_join to defs so it can be called from here
    kthread_join(p->threads[i].tid, 0);
  }
